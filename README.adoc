...
== 📁 Project Structure (src-2.0 Focus)

[source,text]

.
├── JUSTFILE                # Primary build automation runner.
├── README.adoc             # This document.
├── cookbook.adoc           # Comprehensive list of all 'just' recipes.
├── oil-shell/build.oil     # The core build script (replaces 'make-zips').
├── config/                 # Nickel source files.
│   ├── Manifest.nix        # Zotero manifest.json source.
│   └── Updates.nix         # Zotero update JSON source.
├── src-2.0/                # Zotero 7+ codebase (Pure Bootstrapped).
│   ├── Bootstrap.res       # Rescript: Plugin entry point logic (compiles to bootstrap.js).
│   ├── MakeItRed.res       # Rescript: Core DOM and menu manipulation (compiles to MakeItRed.js).
│   ├── prefs.ts            # Zotero preference defaults (kept as simple Mozilla pref file).
│   ├── style.css           # Zotero styling.
│   └── ...
...
== ♿ WCAG 2.3 AAA Compliance (UI/Web)
