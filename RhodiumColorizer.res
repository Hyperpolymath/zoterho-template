/**
 * @file RhodiumColorizer.res
 * @description Core Zotero plugin functionality translated to Rescript.
 * RENAME: Module name updated from MakeItRed to RhodiumColorizer.
 */
...
let log = (msg: string): unit => {
  zotero["debug"](`Rhodium Colorizer: ${msg}`) # RENAME: Update log prefix
}
...
let addToWindow = (window: Js.t<'window>): unit => {
  ...
  | Some(document) =>
    ...
    setAttribute(link, "id", "rhodium-colorizer-stylesheet") # RENAME: Update element ID
    ...
    %raw("window.MozXULElement.insertFTLIfNeeded(\"rhodium-colorizer.ftl\")") # RENAME: Update FTL filename (assuming you rename the file too)

    // 3. Add menu option
    let menuitem = createElement(document, "menuitem")
    setAttribute(menuitem, "id", "rhodium-colorizer-green-instead") # RENAME: Update element ID
    setAttribute(menuitem, "type", "checkbox")
    setAttribute(menuitem, "data-l10n-id", "rhodium-colorizer-green-instead") # RENAME: Update l10n ID
...
let removeFromWindow = (window: Js.t<'window>): unit => {
  ...
  | Some(document) =>
    ...
    let ftlLink = querySelector(document, "link[href$=\"rhodium-colorizer.ftl\"]") # RENAME: Update FTL filename
...
let main = (): Js.Promise.t<unit> => {
  let intensityPref = zotero["Prefs"]["get"]("extensions.rhodium-colorizer.intensity", true) # RENAME: Update preference key
...
// Module exports for the JS shim (RhodiumColorizer.js) to access
...
// Use the full module for the JS output file (RhodiumColorizer.js)
// This exports all functions required by the bootstrap.js
// The names must match the external definitions in Bootstrap.res
// NOTE: Rescript automatically generates the RhodiumColorizer.js file in the same directory.
